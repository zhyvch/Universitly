...
from core.apps.users.selectors.user import DjangoORMUserSelector
...
def send_weekly_emails(self, email_type: Email.EmailType):
    ...
    user_selector = DjangoORMUserSelector()

    try:
        active_users = user_selector.get_active_users()
        ...
        sent_count = email_service.send_mass_mail(
            ...
            names=[user.get_full_name() or user.email[:user.email.index('@')] for user in active_users],
        )
